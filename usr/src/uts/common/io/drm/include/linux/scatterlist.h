...
struct scatterlist {
	struct page	*__page;	/* illumos: struct page * not vm_page * */
...
struct sg_table {
	...
	void		*dmamap;	/* illumos: stub (was bus_dmamap_t) */
};
...
/* illumos Phase 1: sg_page_iter_page() returns NULL */
...
{
	sgl->__page = page;
	sgl->dma_address = 0;	/* illumos: page_to_phys stub returns 0 */
	sgl->offset = offset;
	sgl->length = length;
}
